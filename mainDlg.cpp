...
LRESULT CmainDlg::onCallState(WPARAM wParam, LPARAM lParam)
{
	...
	if (messagesContact) {
		if (call_info->state == PJSIP_INV_STATE_DISCONNECTED) {
			...
			if (call_info->role == PJSIP_ROLE_UAS) {
				pageCalls->Add(call_info->call_id, number, name, MSIP_CALL_IN);
			}
			else {
				pageCalls->Add(call_info->call_id, number, name, MSIP_CALL_OUT);
			}
		}
	}
	if (accountSettings.singleMode) {
...
LRESULT CmainDlg::onBuddyState(WPARAM wParam, LPARAM lParam)
{
	...
	for (int i = 0; i < n; i++) {
		if (contact == (Contact *)list->GetItemData(i)) {
			...
			CString str = contact->info;
			list->SetItemText(i, 2, Translate(str.GetBuffer()));
			found = true;
		}
	}
...
void CmainDlg::OnTimerContactBlink()
{
	...
	for (int i = 0; i < n; i++) {
		...
		if (contact->ringing) {
			list->SetItem(i, 0, LVIF_IMAGE, 0, timerContactBlinkState ? MSIP_CONTACT_ICON_BLANK : contact->image + (contact->starred ? 7 : 0), 0, 0, 0);
			ringing = true;
		}
		else {
			list->SetItem(i, 0, LVIF_IMAGE, 0, contact->image + (contact->starred ? 7 : 0), 0, 0, 0);
